...
for tl=[1,5]%1:numel(Type)
    ...
    for idx = 1:62
        ...
        for kk = GEEPI
            ...
            Vec={};
            dest = fullfile(WholePath,'DiffusionGradient_ION_Random');
...
X2_ = mean(PGR2diff,2)*2;SEM2 = std(PGR2diff,0,2)/sqrt(size(X1_,2));SEM2(1)=SEM2(1)/3;
close all
F = figure;
errorbar((1:10)-0.1,X1_,SEM1,'r-','linewidth',2); hold on;
plot((1:10)-0.1,X1_,'o','MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',10)
xlim([0 11]);ylim([1 9])
