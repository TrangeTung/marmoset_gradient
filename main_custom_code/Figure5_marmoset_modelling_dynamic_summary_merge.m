...
for bl=1:bins
    ...
    FCz = mean(Vec,3);
    dest = fullfile(WholePath,'DiffusionGradient_merge','DynamicModel',Type{tl},num2str(bl,'%02d'));
...
for tl=1:numel(Type)
    ...
    for bl=1:bins
        ...
        FCz = data;
        if tl>=4
            dest_ = fullfile(WholePath,'DiffusionGradient_merge','DynamicModel',Type{1},num2str(bl,'%02d'));
            data_ = csvread(fullfile(dest_,'Connectome.csv'));
            FCz = data_-data;
            
        end
        if tl>=2;FCz = FCz+0.15;end
        %FCz(FCz<0) = 0;
