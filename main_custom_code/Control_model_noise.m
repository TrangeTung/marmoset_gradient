...
NoiseM = zeros(116,116)+randn(116);
dest = fullfile(WholePath,'DiffusionGradient_ION_NoiseControl',['Noise',num2str(nl,'%02d')]);
mkdir(dest);
save(fullfile(dest,'Noise.mat'),'NoiseM');
...
for idx = 1:62
    ...
    for kk = GEEPI
        ...
        sloop = 3; VecP = (beta(sloop,:)'*DM(:,sloop)')';
        predictedFC_Noise_static = reshape(VecP,size(FC));
        ...
        sloop = 3; [~,~,~,~,VecP] = MY_gpuArray_GLM_shuffle_static(V(:)',DesignMatrix,fa,sloop);
        predictedFC_Noiseshuffle_static = reshape(VecP,size(FC));
        ...
        sloop = 3; VecP = (beta(sloop,:)'*DM(:,sloop)')';
        predictedFC_Noise_dynamic = reshape(VecP,size(D));
        ...
        sloop = 3; [~,~,~,~,VecP] = MY_gpuArray_GLM_shuffle(V',DesignMatrix,fa,sloop);
        predictedFC_Noiseshuffle_dynamic = reshape(VecP,size(D));
        %% static save
        dest = fullfile(WholePath,'DiffusionGradient_ION_NoiseControl',['Noise',num2str(nl,'%02d')]);
        ...
        csvwrite(fullfile(dest_,'predictedFC_Noise_static.csv'),predictedFC_Noise_static);
        ...
        csvwrite(fullfile(dest_,'predictedFC_Noiseshuffle_static.csv'),predictedFC_Noiseshuffle_static);
        %% dynamic save
        ...
        hdr.fname = fullfile(dest_,'predictedFC_Noise_dynamic.nii'); 
        spm_write_vol(hdr,predictedFC_Noise_dynamic);
        ...
        hdr.fname = fullfile(dest_,'predictedFC_Noiseshuffle_dynamic.nii'); 
        spm_write_vol(hdr,predictedFC_Noiseshuffle_dynamic);
