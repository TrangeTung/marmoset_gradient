...
try 
    CosView = args.CosView;
    if contains(CosView,'crossline')
       LOC =  args.crossLOCS;
    end
catch
    CosView = [];
...
if ~ strcmp(template,'no#$')
    ...
    if contains(template,'.nii')
        ...
        Template_Img_3D = Template_Img_3D;
        %Template_Img_3D = permute(Template_Img_3D,[1 3  2]);
...
% 3D to 2D ,[1 3 2]
% % % % img_reshape = reshape(permute(img_reslice,[2 1 3]),[size(img_reslice,2) size(img_reslice,1)*numel(slice)]);
img_reshape = reshape(permute(img_reslice,[2 1 3]),[size(img_reslice,2) size(img_reslice,1)*numel(slice)]);
img_reshape = img_reshape/max(img_reshape(:))*255*1.3;
map3D_reshape  = reshape(permute(map3D_reslice,[2 1 3]),[size(map3D_reslice,2) size(map3D_reslice,1)*numel(slice)]);

% % % % map3D_reshape = reshape(permute(map3D_reslice,[2 1 3]),[size(img_reslice,2) size(img_reslice,1)*numel(slice)]);
...
bd = abs(bar_value_normalize(4)-bar_value_normalize(3));
map3D_reshape_positive = double((map3D_reshape(positive_pixel)-bar_value_normalize(3))./bd);
positive_index = find(double(positive_pixel)==1);
tmap(positive_index+numel(map3D_reshape)*0) = 255.0;
tmap(positive_index+numel(map3D_reshape)*1) = 255-map3D_reshape_positive*255;
tmap(positive_index+numel(map3D_reshape)*2) = 255-map3D_reshape_positive*255;

% negative pixels painting
bd=abs(bar_value_normalize(1)-bar_value_normalize(2));
map3D_reshape_negative = double((map3D_reshape(negative_pixel)-bar_value_normalize(2))./bd);
negative_index = find(double(negative_pixel)==1);
tmap(negative_index+numel(map3D_reshape)*0) = 255-abs(map3D_reshape_negative*255);
tmap(negative_index+numel(map3D_reshape)*1) = 255-abs(map3D_reshape_negative*255);
tmap(negative_index+numel(map3D_reshape)*2) = 255;

global f_mask
if ~isempty(f_mask)
    mask_reslice = permute(flip(f_mask(:,:,slice),1),[2 1 3]);
    mask_reshape = reshape(mask_reslice,[size(mask_reslice,2) size(mask_reslice,1)*numel(slice)]);
...
% defaultMap = [gdmap(1:end-1,:);...
%     [1 1 1];[1 1 1];...
%     flipud(drmap(1:end-1,:));];
...
switch flag
    case '-+'
        ...
        ti = insertText(ti,[ceil(leng*0.85),tall/2],TF,'FontSize',min(30*pump,200),'TextColor',[255 255 255],'BoxColor',[0 0 0],'BoxOpacity',0,'AnchorPoint','Center');
