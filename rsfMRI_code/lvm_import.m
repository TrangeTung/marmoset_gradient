...
Fs = unique(num_samples);
t = data.Segment1.data(:,1);
DATA = nan([round(max(t)*Fs),8]);
tn = [0 (1:round(max(t)*Fs))/Fs];

for iter = 1:size(data.Segment1.data,1)
    pin = round(data.Segment1.data(iter,1)*Fs)/Fs;
    DATA(tn==pin,:) = data.Segment1.data(iter,:);
end
[F,~] = fillmissing(DATA,'linear','SamplePoints',tn);
...
Resp1=Resp;
Resp1(Resp1>=4.95)=nan;
Resp1(Resp1<=0.05)=nan;
[Resp,~] = fillmissing(Resp1,'spline','SamplePoints',tn);


if(~isempty(Trig<1));TrigTXT = 'Trigger ON';else;TrigTXT = 'Trigger OFF';end
if(max(ECGw(:)>4));ECGTXT = 'ECGwave ON';else;ECGTXT = 'ECGwave OFF';end
if(max(Resp(:)>4));RespTXT = 'Respwave ON';else;RespTXT = 'Respwave OFF';end

data.TrigTXT=TrigTXT;
data.ECGTXT=ECGTXT;
data.RespTXT=RespTXT;
data.Trig=Trig;
data.ECG=ECGw;
data.Resp=Resp;
data.Time=Time;
